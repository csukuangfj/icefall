...
def get_parser():
    ...
    parser.add_argument(
        "--ctc-loss-scale", type=float, default=0.5, help="ctc loss scale"
    )
    ...
    params = AttributeDict(
        {
            ...
            # for max duration 500
            "batches_per_epoch": 21000,  # 100h: 2192, full libri: 21000
        }
    )
...
def get_joiner_model(params: AttributeDict) -> nn.Module:
    joiner = Joiner(
        encoder_dim=params.encoder_hidden_dim,
        decoder_dim=params.decoder_dim,
...
def compute_loss(
    ...
    ragged_ys = k2.RaggedTensor(ys).to(device)

    with torch.set_grad_enabled(is_training):
        #  pdb.set_trace()
        simple_loss, pruned_loss, encoder_out, encoder_out_lens = model(
            x=feature,
            x_lens=feature_lens,
            y=ragged_ys,
        ...
        nnet_output = model.ctc(encoder_out)
