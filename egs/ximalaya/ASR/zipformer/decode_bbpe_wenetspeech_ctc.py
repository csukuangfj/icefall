...
from wenetspeech import WenetSpeechAsrDataModule
...
def main():
    ...
    params.res_dir = params.exp_dir / "wenetspeech" / params.decoding_method
    ...
    if params.decoding_method in [
        ...
        bpe_model.load(str(params.lang_dir / "bbpe.model"))
    else:
        H = None
        bpe_model = None
    ...
    weneetspeech = WenetSpeechAsrDataModule(args)
    ...
    test_net_cuts = weneetspeech.test_net_cuts()
    test_net_cuts = test_net_cuts.filter(remove_short_utt)
    test_net_dl = weneetspeech.valid_dataloaders(test_net_cuts)

    test_meeting_cuts = weneetspeech.test_meeting_cuts()
    test_meeting_cuts = test_meeting_cuts.filter(remove_short_utt)
    test_meeting_dl = weneetspeech.test_dataloaders(test_meeting_cuts)
